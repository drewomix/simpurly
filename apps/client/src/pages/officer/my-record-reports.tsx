...
export default function MyOfficersLogs({ reports: data }: Props) {
  ...
  return (
    <Layout permissions={{ permissions: [Permissions.Leo] }} className="mark43-cad-layout">
      <Title renderLayoutTitle={false}>{t("myRecordReports")}</Title>

      <Mark43OfficerLayout
        label={t("officer")}
        title={t("myRecordReports")}
        subtitle={t("myRecordReportsDescription")}
      >
        {data.totalCount <= 0 ? (
          <p className="mark43-cad__empty">{t("noReportsCreated")}</p>
        ) : (
          <Table
